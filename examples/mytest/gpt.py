...
class GPTWeights(object):
    ...
    def load(self, ckpt_path, tensor_para_rank, pipeline_para_rank):
        ...
        wpe = torch.from_numpy(np.fromfile(ckpt_path + "/model.wpe.bin", dtype=self.weights_data_type)).reshape(-1, self.global_hidden_units)
        ...
        #print(wpe.size())
        #print(wpe)
        #wpe = torch.ones((1024, 1024)) #, dtype=torch.uint8)
        # wpe = torch.softmax(wpe)
        #_d = [list(range(1024))] * 1024
        #wpe = torch.Tensor(_d)
        #print(wpe.size())
        ###########################
        # _hidden_size = 1024
        #spe = SinusoidalPositionalEmbedding(_hidden_size)
        #_position_ids = torch.arange(0, _hidden_size).unsqueeze(0)
        #wpe = spe.forward(_position_ids)
        #wpe = wpe.squeeze(0)
        ###########################
        w.append(wpe)
        ...
        for i in range(len(w)):
            if w[i].nelement() > 0:
                print(i, w[i].shape, self.w[i].shape)
...
class GPT(nn.Module):
    ...
    def forward(self,
                ...
                output_len,
                beam_width=1,
                option_last_ids: torch.Tensor=None,
                option_last_counts: torch.Tensor=None,
                top_k=None,
                top_p=None,
        ...
        if option_last_ids is not None or option_last_counts is not None:
            assert option_last_ids is not None, "option_last_ids is None"
            assert option_last_counts is not None, "option_last_counts is None"
            option_last_ids = option_last_ids.cuda(self.device)
            option_last_counts = option_last_counts.cuda(self.device)
        ...
        """
        outputs1 = self.model.forward(start_ids,
                                     start_lengths,
                                     1,
...
OPENAI_GPT2_START_ID = 50048
OPENAI_GPT2_END_ID = 50048
